...
#include "GameOver.h"
std::string DictLayer2::language;
std::string DictLayer2::level;
using namespace tinyxml2;
DictLayer2::DictLayer2() :
dict_signal(0),
atomicHeight(0.0f),
atomicWidth(0.0f),
scale(0.0f),
...
void DictLayer2::initLanguage(const char* level){
...
void DictLayer2::initDict(const char* language, const char* level) {
	//char* errMsg;
	sqlite3_open(
...
void DictLayer2::createDictLabel(const char* language, const char* level){
	if (strcmp(language, "en") == 0)
...
void DictLayer2::createMorseCodeSprites(){
...
void DictLayer2::initGroupTime_Score(){
	groupID++;
...
void DictLayer2::execSignal(Ref * pObjc, Widget::TouchEventType type) {
	switch (type){
		...
		if (signalPosition.size() > 0){
			if (((Button*)pObjc)->getTag() == 1)
			{
				if (signalPosition.back().isDit){
					...
					ditScoreLabel->runAction(Sequence::create(Spawn::createWithTwoActions(MoveTo::create(score_one_move_duration, Vec2(ditNormalButton->getPosition().x, ditNormalButton->getPosition().y + 50)), FadeOut::create(score_one_fadeout_duration)), CCCallFuncN::create(CC_CALLBACK_1(DictLayer2::deleteObject, this)), NULL));
					groupScore += 1;
					SimpleAudioEngine::getInstance()->playEffect(ditEffect);
					morseCodeSprites.at(signalPosition.back().position)->runAction(Sequence::create(ScaleTo::create(0.1f, (1 / scale)*1.5), Spawn::create(ScaleTo::create(0.1f, 1 / scale), TintBy::create(0, -255, -142, 0), NULL), NULL));
					...
					morseCodeSprites.at(signalPosition.back().position)->runAction(Sequence::create(ScaleTo::create(0.1f, (1 / scale)*1.5), Spawn::create(ScaleTo::create(0.1f, 1 / scale), TintBy::create(0, 0, -255, -255), NULL), NULL));
					signalPosition.pop_back();
					if (signalPosition.size() <= 0){
						complete();
					}
			...
			if (((Button*)pObjc)->getTag() == 2){
				if (!signalPosition.back().isDit){
					...
					dahScoreLabel->runAction(Sequence::create(Spawn::createWithTwoActions(MoveTo::create(score_one_move_duration, Vec2(dahNormalButton->getPosition().x, dahNormalButton->getPosition().y + 50)), FadeOut::create(score_one_fadeout_duration)), CCCallFuncN::create(CC_CALLBACK_1(DictLayer2::deleteObject, this)), NULL));
					groupScore += 1;
...
void DictLayer2::complete(){
	...
	if (groupErrorTimes == 0 && groupMissTimes == 0){
		...
		perfect->runAction(Sequence::create(Spawn::createWithTwoActions(ScaleTo::create(0.5f, 1.2), FadeIn::create(score_one_fadeout_duration)), CCCallFuncN::create(CC_CALLBACK_1(DictLayer2::completeCallback, this)), NULL));
	}
		...
		great->runAction(Sequence::create(Spawn::createWithTwoActions(ScaleTo::create(0.5f, 1.2), FadeIn::create(score_one_fadeout_duration)), CCCallFuncN::create(CC_CALLBACK_1(DictLayer2::completeCallback, this)), NULL));
		...
		good->runAction(Sequence::create(Spawn::createWithTwoActions(ScaleTo::create(0.5f, 1.2), FadeIn::create(score_one_fadeout_duration)), CCCallFuncN::create(CC_CALLBACK_1(DictLayer2::completeCallback, this)), NULL));
	}
}
void DictLayer2::completeCallback(Node* sender){
	group_scores.push_back({ groupID, groupLimitTime, groupFinishTime, groupTimes, groupErrorTimes, groupMissTimes, groupScore });
	sender->removeFromParent();
	this->scheduleOnce(schedule_selector(DictLayer2::createDict), 0.1f);
}
void DictLayer2::deleteObject(Node* sender){
	sender->removeFromParent();
}
void DictLayer2::update(float dt){
	groupFinishTime += dt;
	LoadingBar* timeBar = static_cast<LoadingBar*>(this->getChildByTag(timeBar_tag));
...
void DictLayer2::createDict(float dt) {
...
void DictLayer2::createButton(){
	...
	ditNormalButton->addTouchEventListener(CC_CALLBACK_2(DictLayer2::execSignal, this));
	dahNormalButton->addTouchEventListener(CC_CALLBACK_2(DictLayer2::execSignal, this));
...
std::string DictLayer2::convertStr(const char* str) {
	std::string out = "";
	for (int i = 0; str[i]; i++) {
