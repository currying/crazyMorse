...
bool ShareScene::init()
{
	...
	listener->onKeyReleased = CC_CALLBACK_2(ShareScene::onKeyReleased, this);
	_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);
	initData();
	createListView();
	return true;
}
void ShareScene::initData() {
	sqlite3_open(
...
void ShareScene::createListView() {
	...
	listView->addEventListener(
		...
		ShareScene::selectedItemEvent, this));
	listView->addEventListener(
		(ui::ListView::ccScrollViewCallback)CC_CALLBACK_2(
		ShareScene::selectedItemEventScrollView, this));
...
void ShareScene::selectedItemEvent(Ref * pObjc, ListView::EventType type) {
	switch (type) {
	case cocos2d::ui::ListView::EventType::ON_SELECTED_ITEM_END: {
		ListView* listView = static_cast<ListView*>(pObjc);
		currentIndex = listView->getCurSelectedIndex();
		this->runAction(Sequence::create(CallFunc::create(CC_CALLBACK_0(ShareScene::share, this)), NULL));
		break;
	}
	default:
		break;
...
void ShareScene::selectedItemEventScrollView(Ref* pSender, ui::ScrollView::EventType type){
}
void ShareScene::share() {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID)
	ShareHelper::share("",commons.at(currentIndex).code.c_str(),"");
#endif
}
void ShareScene::onKeyReleased(EventKeyboard::KeyCode keyCode, Event* event)
{
